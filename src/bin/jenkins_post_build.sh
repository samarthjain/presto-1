...
PRESTO_VERSION=`ls presto-server/target/*.tar.gz  | xargs -n 1 basename | cut -d'-' -f3| cut -d'.' -f1`
...
/apps/python/bin/python /usr/local/bin/assume-role -a arn:aws:iam::219382154434:role/BDP_JENKINS_ROLE -r us-east-1 aws s3 cp presto-cli/target/presto-cli*executable*.jar $CLI_BUILD_PATH
...
if [[ "${STAGE_ON_S3:-false}" == "true" ]]; then
  ...
  /apps/python/bin/python /usr/local/bin/assume-role -a arn:aws:iam::219382154434:role/BDP_JENKINS_ROLE -r us-east-1 aws s3 cp presto-cli/target/presto-cli*executable*.jar $CLI_DIR
  /apps/python/bin/python /usr/local/bin/assume-role -a arn:aws:iam::219382154434:role/BDP_JENKINS_ROLE -r us-east-1 aws s3 cp presto-server/target/*tar.gz $SERVER_DIR
else
